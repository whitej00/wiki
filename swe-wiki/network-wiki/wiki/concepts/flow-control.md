...
[[TCP]]가 제공하는 **속도 매칭 서비스(Speed-matching Service)**로, 송신자가 수신자의 버퍼를 오버플로시키지 않도록 전송률을 조절하는 메커니즘이다.
...
| | 흐름 제어 | [[congestion-control\|혼잡 제어]] |
|---|---------|------------|
| **보호 대상** | 수신자 버퍼 | 네트워크 |
...
[[UDP]]는 흐름 제어를 **제공하지 않는다**. 수신 측 소켓에 유한 크기 버퍼가 있으며, 애플리케이션이 충분히 빠르게 읽지 않으면 세그먼트가 **폐기**된다.
