...
네트워크 혼잡 — 너무 많은 소스가 너무 빠르게 데이터를 전송하여 네트워크가 처리할 수 없는 상태 — 을 감지하고, 송신자의 전송률을 조절하여 혼잡을 방지하거나 완화하는 메커니즘이다. [[flow-control|흐름 제어]]가 수신자 보호인 반면, 혼잡 제어는 **네트워크 전체의 이익**을 위한 것이다.
...
[[TCP]]가 이 접근을 사용: 세그먼트 손실(타임아웃 또는 3중복 ACK)을 혼잡의 신호로 해석하고 윈도우 크기를 줄인다.
