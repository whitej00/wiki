...
### 4.1 [[layers/network|네트워크 계층]] 개관
- 네트워크 계층은 **모든 호스트와 라우터**에 존재 ([[layers/transport|전송]]/[[layers/application|애플리케이션]] 계층과 달리)
- 두 가지 핵심 기능: **[[forwarding-routing|포워딩(Forwarding, data plane)과 라우팅(Routing, control plane)]]**
- 포워딩: 라우터 로컬 동작, 입력→출력 포트 전달 (나노초, 하드웨어)
- 라우팅: 네트워크 전체 경로 결정 (초 단위, 소프트웨어)
- **포워딩 테이블**: 라우터의 핵심 데이터 구조
- Control plane 접근: (1) 전통적 — 각 라우터에서 라우팅 알고리즘 실행, (2) [[sdn-control-plane|SDN]] — 원격 컨트롤러가 포워딩 테이블 계산·배포
...
### 4.2 [[router-architecture|라우터 내부]]
...
### 4.3 IP: [[IPv4]], [[ip-addressing|주소]], [[IPv6]]
#### [[IPv4]] 데이터그램
- **20바이트 헤더** (옵션 없을 때)
...
### 4.4 [[generalized-forwarding|일반화된 포워딩]]과 SDN
...
### 4.5 [[middleboxes|미들박스]]
- RFC 3234 정의: "소스와 목적지 사이 데이터 경로에서 표준 IP 라우터 기능 외의 기능을 수행하는 중간 장치"
- 3가지 유형: (1) [[nat|NAT]], (2) 보안 서비스(방화벽/IDS), (3) 성능 향상(압축/캐싱/로드밸런싱)
- **NFV(Network Function Virtualization)**: [[sdn-control-plane|SDN]] 접근으로 미들박스 기능을 소프트웨어로 구현
- **IP 모래시계(IP Hourglass)**: 네트워크 계층에 IP 하나만 존재 — 인터넷 성장의 핵심
- **[[end-to-end-principle|End-to-end 논거]]**: 지능은 네트워크 코어가 아닌 종단에 배치 [Saltzer 1984, RFC 1958]

## 다룬 토픽 목록
- [[forwarding-routing]], [[router-architecture]], [[ip-addressing]], [[IPv4]], [[IPv6]], [[DHCP]], [[NAT]], [[packet-scheduling]], [[generalized-forwarding]], [[middleboxes]], [[network]]
