...
### 6.4 스위치 LAN
- **[[mac-address|MAC 주소]]**: 6바이트, 평면적, IEEE 관리, 브로드캐스트=FF-FF-FF-FF-FF-FF
- **[[ARP]]**: IP→MAC 변환, 브로드캐스트 질의/유니캐스트 응답, TTL 20분, 같은 서브넷 내에서만
- 서브넷 간 전송: 프레임의 MAC 주소는 매 홉마다 변경, IP 주소는 불변
- **[[Ethernet|이더넷]]**: 프레임 구조(Preamble+Dest+Src+Type+Data+CRC), 비연결/비신뢰, CSMA/CD
- 이더넷 진화: 버스→허브 스타→스위치 스타, 10M→100M→1G→10G→40G
- **[[link-layer-switching|링크 계층 스위치]]**: self-learning, filtering/forwarding, plug-and-play, switch table
- 스위치 vs 라우터: L2 vs L3, 브로드캐스트 스톰 취약 vs 최적 경로
- **[[vlan|VLAN]]**: 포트 기반 논리적 LAN 분할, 802.1Q 트렁킹(4바이트 태그, 12비트 VLAN ID)

### 6.5 링크 가상화: [[mpls|MPLS]]
- MPLS 헤더: [[Ethernet]]과 IP 사이, 20비트 라벨+Exp+S+TTL
- 라벨 스위치 라우터: 라벨 기반 포워딩 ([[ip-addressing|IP 주소]] 참조 불필요)
- 핵심 가치: 트래픽 엔지니어링(다중 경로, 부하 분산), 빠른 장애 복구, VPN

### 6.6 [[data-center-networking|데이터 센터 네트워킹]]
- 구조: 블레이드→랙→TOR→Tier-2→Tier-1→Access Router→Border Router
- 로드 밸런서: [[nat|NAT]] 유사 기능, Layer-4 스위치
- 대역폭 문제: 계층 상위로 갈수록 공유 → 호스트 간 대역폭 감소
- 해결: Clos 네트워크(고밀도 상호 연결), ECMP
- 트렌드: 커머디티 스위치, [[sdn-control-plane|SDN]] 중앙 제어, VM 가상화, RDMA, MDC

### 6.7 웹 페이지 요청의 하루
- [[DHCP]]→[[ARP]]→[[DNS]]→[[TCP]] 3-way handshake→[[HTTP]] GET/응답
- [[protocol-layers|프로토콜 스택]] 전체를 관통하는 통합 시나리오 (24단계)
...
- [[ARP]] — ARP 프로토콜
- [[mac-address]] — MAC 주소
- [[error-detection-correction]] — 오류 검출/정정
- [[multiple-access]] — 다중 접속 프로토콜
- [[link-layer-switching]] — 링크 계층 스위칭
- [[vlan]] — VLAN
- [[mpls]] — MPLS
- [[data-center-networking]] — 데이터 센터 네트워킹
