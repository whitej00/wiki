...
- 다른 위키 페이지를 참조할 때 [페이지명](#페이지명) 형식 사용 (Obsidian 위키링크)
- 코드 예제를 적극적으로 포함
...
4. 새로 만든 페이지에서 기존 페이지로, 기존 페이지에서 새 페이지로 [위키링크](#위키링크)를 추가한다
...
- [위키링크](#위키링크)가 걸려 있지만 실제 페이지가 없는 것 찾기
...
## Concepts
- [closure](#closure) — 함수가 자신의 렉시컬 스코프를 기억하는 것
- [event-loop](#event-loop) — JavaScript 비동기 처리의 핵심 메커니즘

## Languages
- [python](#python) — 문법, 관용구, 생태계 정리

## Tools
- [docker](#docker) — 컨테이너 기반 개발 환경
...
```
