...
- `strace`로 추적 가능하지만 ptrace 기반이라 오버헤드 큼 → [[bpf]] 기반 `syscount` 권장
...
### Extended BPF
→ [[bpf]] 참조. 2013년 이후 확장되어 일반 목적 인-커널 실행 환경이 됨.
...
- [[cpu-performance]] — 스케줄러 상세
- [[memory-performance]] — 가상 메모리 상세
